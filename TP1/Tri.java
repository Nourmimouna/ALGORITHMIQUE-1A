...
public class Tri{
	...
	return(recDichotomie(tableau,x,i,j));   
    }
    ...
    public static int recDichotomie(int[] tableau, int x, int i, int j) {
        ...
        if(tableau[m]<x){
        	return recDichotomie(tableau, x , m+1, j); 
        	}
        return recDichotomie(tableau, x, i, m-1); 
    ...
    public static int trichotomie(int[] tableau, int x) {
        ...
        return recTrichotomie(tableau,  x, i, j-1) ;
    }
    ...
    public static int recTrichotomie(int[] tableau, int x, int i, int j) {
        ...
        if(x<tableau[m1]){
        	return recTrichotomie(tableau, x , i, m1-1); 
        	}
        	
        if(x>tableau[m2]){
        	return recTrichotomie(tableau, x , m2+1, j); 
        	}
        return recTrichotomie(tableau, x , m1+1, m2-1); 
    }
   ...
   public static int compteDichotomie(int[] tableau, int x) {
    int counter = 0;
    return recCompteDichotomie(tableau, x, 0, tableau.length - 1, counter);
}

public static int recCompteDichotomie(int[] tableau, int x, int i, int j, int counter) {
    counter++; // Incrémente le compteur pour chaque appel de la méthode récursive
    if (i > j) {
        return counter; // Retourne le nombre total de comparaisons
    }
    int m = (i + j) / 2;
    if (tableau[m] == x) {
        return counter; // Retourne le nombre total de comparaisons
    ...
    if (tableau[m] < x) {
        return recCompteDichotomie(tableau, x, m + 1, j, counter);
    }
    return recCompteDichotomie(tableau, x, i, m - 1, counter);
}

public static int compteTrichotomie(int[] tableau, int x) {
    int counter = 0;
    return recCompteTrichotomie(tableau, x, 0, tableau.length - 1, counter);
}

public static int recCompteTrichotomie(int[] tableau, int x, int i, int j, int counter) {
    counter++; // Incrémente le compteur pour chaque appel de la méthode récursive
    if (i > j) {
        return counter; // Retourne le nombre total de comparaisons
    }
    int m1 = i + (j - i) / 3;
    int m2 = i + 2 * (j - i) / 3;
    if (tableau[m1] == x || tableau[m2] == x) {
        return counter; // Retourne le nombre total de comparaisons
    }
    if (x < tableau[m1]) {
        return recCompteTrichotomie(tableau, x, i, m1 - 1, counter);
    } else if (x > tableau[m2]) {
        return recCompteTrichotomie(tableau, x, m2 + 1, j, counter);
    } else {
        return recCompteTrichotomie(tableau, x, m1 + 1, m2 - 1, counter);
    }
}
